...
# RSA公钥 - getConsts(1) DER格式（与getConsts(3) PEM不同！）
...
def uns_email_login():
    ...
    try:
        ...
        # Build device data (from C60311n0 interceptor)
        device_data = {
        ...
        # Hex-encode SM4 key/IV for device data (matching C60463z0.m29471a)
        sm4_key_hex = bytes.fromhex(dev_sm4_key).hex().lower()
        sm4_iv_hex = bytes.fromhex(dev_sm4_iv).hex().lower()

        # Request body = all interceptor fields combined
        body = {
