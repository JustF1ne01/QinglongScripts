...
# fmt: off
push_config = {
    'HITOKOTO': os.getenv('HITOKOTO', 'true').lower() == 'true',  # 启用一言（随机句子）
    ...
    'CONSOLE': os.getenv('CONSOLE', 'false').lower() == 'true',  # 控制台输出
    ...
    'GOBOT_URL': os.getenv('GOBOT_URL', ''),                     # go-cqhttp
                                                                 # 推送到个人QQ：http://127.0.0.1/send_private_msg
                                                                 # 群：http://127.0.0.1/send_group_msg
    'GOBOT_QQ': os.getenv('GOBOT_QQ', ''),                       # go-cqhttp 的推送群或用户
                                                                 # GOBOT_URL 设置 /send_private_msg 时填入 user_id=个人QQ
                                                                 #               /send_group_msg   时填入 group_id=QQ群
    'GOBOT_TOKEN': os.getenv('GOBOT_TOKEN', ''),                 # go-cqhttp 的 access_token

    'GOTIFY_URL': os.getenv('GOTIFY_URL', ''),                   # gotify地址,如https://push.example.de:8080
    'GOTIFY_TOKEN': os.getenv('GOTIFY_TOKEN', ''),               # gotify的消息应用token
    'GOTIFY_PRIORITY': int(os.getenv('GOTIFY_PRIORITY', 0)),     # 推送消息优先级,默认为0

    'IGOT_PUSH_KEY': os.getenv('IGOT_PUSH_KEY', ''),             # iGot 聚合推送的 IGOT_PUSH_KEY

    'PUSH_KEY': os.getenv('PUSH_KEY', ''),                       # server 酱的 PUSH_KEY，兼容旧版与 Turbo 版

    'DEER_KEY': os.getenv('DEER_KEY', ''),                       # PushDeer 的 PUSHDEER_KEY
    'DEER_URL': os.getenv('DEER_URL', ''),                       # PushDeer 的 PUSHDEER_URL

    'CHAT_URL': os.getenv('CHAT_URL', ''),                       # synology chat url
    'CHAT_TOKEN': os.getenv('CHAT_TOKEN', ''),                   # synology chat token

    'PUSH_PLUS_TOKEN': os.getenv('PUSH_PLUS_TOKEN', ''),         # pushplus 推送的用户令牌
    'PUSH_PLUS_USER': os.getenv('PUSH_PLUS_USER', ''),           # pushplus 推送的群组编码
    'PUSH_PLUS_TEMPLATE': os.getenv('PUSH_PLUS_TEMPLATE', 'html'),  # pushplus 发送模板，支持html,txt,json,markdown,cloudMonitor,jenkins,route,pay
    'PUSH_PLUS_CHANNEL': os.getenv('PUSH_PLUS_CHANNEL', 'wechat'),  # pushplus 发送渠道，支持wechat,webhook,cp,mail,sms
    'PUSH_PLUS_WEBHOOK': os.getenv('PUSH_PLUS_WEBHOOK', ''),     # pushplus webhook编码，可在pushplus公众号上扩展配置出更多渠道
    'PUSH_PLUS_CALLBACKURL': os.getenv('PUSH_PLUS_CALLBACKURL', ''),  # pushplus 发送结果回调地址，会把推送最终结果通知到这个地址上
    'PUSH_PLUS_TO': os.getenv('PUSH_PLUS_TO', ''),               # pushplus 好友令牌，微信公众号渠道填写好友令牌，企业微信渠道填写企业微信用户id

    'WE_PLUS_BOT_TOKEN': os.getenv('WE_PLUS_BOT_TOKEN', ''),     # 微加机器人的用户令牌
    'WE_PLUS_BOT_RECEIVER': os.getenv('WE_PLUS_BOT_RECEIVER', ''),  # 微加机器人的消息接收者
    'WE_PLUS_BOT_VERSION': os.getenv('WE_PLUS_BOT_VERSION', 'pro'),  # 微加机器人的调用版本

    'QMSG_KEY': os.getenv('QMSG_KEY', ''),                       # qmsg 酱的 QMSG_KEY
    'QMSG_TYPE': os.getenv('QMSG_TYPE', ''),                     # qmsg 酱的 QMSG_TYPE

    'QYWX_ORIGIN': os.getenv('QYWX_ORIGIN', ''),                 # 企业微信代理地址

    'QYWX_AM': os.getenv('QYWX_AM', ''),                         # 企业微信应用

    'QYWX_KEY': os.getenv('QYWX_KEY', ''),                       # 企业微信机器人

    'TG_BOT_TOKEN': os.getenv('TG_BOT_TOKEN', ''),               # tg 机器人的 TG_BOT_TOKEN，例：1407203283:AAG9rt-6RDaaX0HBLZQq0laNOh898iFYaRQ
    'TG_USER_ID': os.getenv('TG_USER_ID', ''),                   # tg 机器人的 TG_USER_ID，例：1434078534
    'TG_API_HOST': os.getenv('TG_API_HOST', ''),                 # tg 代理 api
    'TG_PROXY_AUTH': os.getenv('TG_PROXY_AUTH', ''),             # tg 代理认证参数
    'TG_PROXY_HOST': os.getenv('TG_PROXY_HOST', ''),             # tg 机器人的 TG_PROXY_HOST
    'TG_PROXY_PORT': os.getenv('TG_PROXY_PORT', ''),             # tg 机器人的 TG_PROXY_PORT

    'AIBOTK_KEY': os.getenv('AIBOTK_KEY', ''),                   # 智能微秘书 个人中心的apikey 文档地址：http://wechat.aibotk.com/docs/about
    'AIBOTK_TYPE': os.getenv('AIBOTK_TYPE', ''),                 # 智能微秘书 发送目标 room 或 contact
    'AIBOTK_NAME': os.getenv('AIBOTK_NAME', ''),                 # 智能微秘书 发送群名 或者好友昵称和type要对应好

    'SMTP_SERVER': os.getenv('SMTP_SERVER', ''),                 # SMTP 发送邮件服务器，形如 smtp.exmail.qq.com:465
    'SMTP_SSL': os.getenv('SMTP_SSL', 'false').lower() == 'true',  # SMTP 发送邮件服务器是否使用 SSL，填写 true 或 false
    'SMTP_EMAIL': os.getenv('SMTP_EMAIL', ''),                   # SMTP 收发件邮箱，通知将会由自己发给自己
    'SMTP_PASSWORD': os.getenv('SMTP_PASSWORD', ''),             # SMTP 登录密码，也可能为特殊口令，视具体邮件服务商说明而定
    'SMTP_NAME': os.getenv('SMTP_NAME', ''),                     # SMTP 收发件人姓名，可随意填写

    'PUSHME_KEY': os.getenv('PUSHME_KEY', ''),                   # PushMe 的 PUSHME_KEY
    'PUSHME_URL': os.getenv('PUSHME_URL', ''),                   # PushMe 的 PUSHME_URL

    'CHRONOCAT_QQ': os.getenv('CHRONOCAT_QQ', ''),               # qq号
    'CHRONOCAT_TOKEN': os.getenv('CHRONOCAT_TOKEN', ''),         # CHRONOCAT 的token
    'CHRONOCAT_URL': os.getenv('CHRONOCAT_URL', ''),             # CHRONOCAT的url地址

    'WEBHOOK_URL': os.getenv('WEBHOOK_URL', ''),                 # 自定义通知 请求地址
    'WEBHOOK_BODY': os.getenv('WEBHOOK_BODY', ''),               # 自定义通知 请求体
    'WEBHOOK_HEADERS': os.getenv('WEBHOOK_HEADERS', ''),         # 自定义通知 请求头
    'WEBHOOK_METHOD': os.getenv('WEBHOOK_METHOD', ''),           # 自定义通知 请求方法
    'WEBHOOK_CONTENT_TYPE': os.getenv('WEBHOOK_CONTENT_TYPE', ''),  # 自定义通知 content-type

    'NTFY_URL': os.getenv('NTFY_URL', ''),                       # ntfy地址,如https://ntfy.sh
    'NTFY_TOPIC': os.getenv('NTFY_TOPIC', ''),                   # ntfy的消息应用topic
    'NTFY_PRIORITY': os.getenv('NTFY_PRIORITY', '3'),            # 推送消息优先级,默认为3

    'WXPUSHER_APP_TOKEN': os.getenv('WXPUSHER_APP_TOKEN', ''),   # wxpusher 的 appToken 官方文档: https://wxpusher.zjiecode.com/docs/ 管理后台: https://wxpusher.zjiecode.com/admin/
    'WXPUSHER_TOPIC_IDS': os.getenv('WXPUSHER_TOPIC_IDS', ''),   # wxpusher 的 主题ID，多个用英文分号;分隔 topic_ids 与 uids 至少配置一个才行
    'WXPUSHER_UIDS': os.getenv('WXPUSHER_UIDS', ''),             # wxpusher 的 用户ID，多个用英文分号;分隔 topic_ids 与 uids 至少配置一个才行
}
# fmt: on
...
# 兼容：如果未设置 TG_USER_ID 但设置了 TG_CHAT_ID，则使用 TG_CHAT_ID
if not push_config.get('TG_USER_ID') and os.getenv('TG_CHAT_ID'):
    push_config['TG_USER_ID'] = os.getenv('TG_CHAT_ID')
